...
impl GraphHandler {
    fn create_graph_design(graph_name: &str) -> Result<Design, CommonErrors> {
        ...
        design.add_program("GraphDesignProgram", move |_design_instance, builder| {
            builder.with_run_action(graph_action);
            Ok(())
        });
    ...
    fn graph_parallel_flows(design: &Design) -> Box<LocalGraphAction> {
        let mut graph_builder = LocalGraphActionBuilder::new();
...
impl Scenario for CorrectGraph {
    ...
    fn run(&self, input: &str) -> Result<(), String> {
        ...
        let orch = Orchestration::new()
            .add_design(GraphHandler::create_graph_design(&graph_name).expect("Failed to create design"))
            .design_done();
...
// Chose design from TestInput String
fn choose_design(name: &str) -> Result<Design, CommonErrors> {
    match name {
        "two_steps" => graph_in_sequence(),
        "two_programs" => two_programs(),
        "concurrency" => graph_in_concurrency(),
        "dedicated" => graph_with_dedicated(),
...
fn graph_in_sequence() -> Result<Design, CommonErrors> {
    ...
    let graph1 = graph_builder1.add_edges(n0, &[n1]).build(&design);
    ...
    let graph2 = graph_builder2.add_edges(n2, &[n3]).build(&design);

    // Create a program with some actions
    design.add_program("GraphDesignProgram", move |_design_instance, builder| {
        builder.with_run_action(SequenceBuilder::new().with_step(graph1).with_step(graph2).build());
        Ok(())
    });
...
fn graph_in_concurrency() -> Result<Design, CommonErrors> {
    ...
    let graph2 = graph_builder2.add_edges(n2, &[n3]).add_edges(n3, &[n4]).build(&design);

    // Create a program with some actions
    design.add_program("GraphDesignProgram", move |design_instance, builder| {
        builder.with_run_action(ConcurrencyBuilder::new().with_branch(graph1).with_branch(graph2).build(design_instance));
        Ok(())
    });
...
fn two_programs() -> Result<Design, CommonErrors> {
    ...
    // Create a program with some actions
    design.add_program("GraphDesignProgram", move |_design_instance, builder| {
        builder.with_run_action(graph1);
        Ok(())
    });
    design.add_program("GraphDesignProgram2", move |_design_instance, builder| {
        builder.with_run_action(graph2);
        Ok(())
    });
...
fn graph_with_dedicated() -> Result<Design, CommonErrors> {
    ...
    let n0 = graph_builder.add_node(JustLogAction::new("node0"));
    let n1 = graph_builder.add_node(Invoke::from_tag(&sync_tag_1, design.config()));
    let n2 = graph_builder.add_node(Invoke::from_tag(&sync_tag_2, design.config()));
    let n3 = graph_builder.add_node(Invoke::from_tag(&sync_tag_3, design.config()));
    let n4 = graph_builder.add_node(Invoke::from_tag(&sync_tag_4, design.config()));
    let n5 = graph_builder.add_node(Invoke::from_tag(&sync_tag_5, design.config()));
...
impl Scenario for IntegrationGraph {
    ...
    fn run(&self, input: &str) -> Result<(), String> {
        let logic = TestInput::new(input);
        let design_name = logic.design_name.expect("design_name is required in --input");

        let builder = Runtime::from_json(input)?;
        let mut rt = builder.build();

        let orch = Orchestration::new()
            .add_design(choose_design(&design_name).expect("Failed to create design"))
            .design_done();
...
impl Scenario for DedicatedGraph {
    ...
    fn run(&self, input: &str) -> Result<(), String> {
        ...
        let mut orch = Orchestration::new()
            .add_design(graph_with_dedicated().expect("Failed to create design"))
            .design_done();
...
pub fn graph_scenario_group() -> Box<dyn ScenarioGroup> {
    Box::new(ScenarioGroupImpl::new(
        "graphs",
        vec![
            Box::new(CorrectGraph),
            Box::new(InvalidGraph),
