...
async fn blocking_task(id: String, block_condition: Arc<(Condvar, Mutex<bool>)>, notifier: ThreadReadyNotifier) {
        ...
        let _ = rt.block_on(async move {
            ...
            for dedicated_worker in dedicated_workers {
                ...
                joiner.add_handle(spawn_on_dedicated(
                    blocking_task(dedicated_worker.id.clone(), block_condition.clone(), notifier),
                    unique_worker_id,
                ));
            }
...
impl Scenario for SpawnToUnregisteredWorker {
    ...
    fn run(&self, input: &str) -> Result<(), String> {
        ...
        let _ = rt.block_on(async move {
            let unregistered_worker_id = "unregistered_worker";
            let unique_worker_id = UniqueWorkerId::from(unregistered_worker_id);
            let _ = spawn_on_dedicated(non_blocking_task(unregistered_worker_id.to_string()), unique_worker_id).await;
            Ok(0)
        });
...
pub fn dedicated_worker_group() -> Box<dyn ScenarioGroup> {
    Box::new(ScenarioGroupImpl::new(
        "dedicated_worker",
        vec![Box::new(OnlyDedicatedWorkers), Box::new(SpawnToUnregisteredWorker)],
        vec![],
    ))
}
